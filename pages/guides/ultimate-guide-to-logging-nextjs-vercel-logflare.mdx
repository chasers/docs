...
export const meta = {
  ...
  lastEdited: '2020-06-17T20:28:34.000Z',
...
In this guide we're going to walk through the ideal logging setup for a Next.js app hosted with Vercel. We're going to get Vercel's backend system to send CDN and lambda infrastructure logs to Logflare. We're also going to setup Pino to easily log JSON data. And we're going to search our newly generated logs with Logflare. Logflare automatically parses the stringy JSON log lines for you making them easily queryable with the Logflare Query Language (or even straight SQL).

Vercel has put a lot of work into enabling the easy collection of logs via their Lambda function. In under a second your Vercel logs will be streamed to your browser with Logflare. In under 5 seconds they will be inserted into BigQuery and ready to query!

The only issue with this setup is that Vercel truncates log lines at 4kb. Be mindful of how much you log in each request.

The best thing about Vercel logging is that your log messages are automatically associated with the context of the request because they're embedded in the `lambdaMessage`. This lets us easily correlate the end user experience with our application execution.
...
## Using your Preferred Logging Vendor

This setup is possible with any other vendor. You can easily use Vercel's integration examples and write your own private integration to setup custom log drains.

<Note>Log drains must be setup via an integration.</Note>

We're using Logflare for this example because the Logflare integration was optimized for Vercel.

## Example Repo

You can reference this example repo anytime.

<https://github.com/Logflare/pino-logflare-nextjs-vercel-example>

## Setup the Logflare Vercel Integration
...
[Visit your Logflare dashboard](https://logflare.app/dashboard)

## Install Pino

Install `pino`.

### The Pino `base` Object

Set the fields to be included with every log event in the `base` object when you instantiate Pino.

### Log the Github Commit
...
And we're finally setup to actually log some stuff from our application! We can just setup our `logger` like:

```tsx
const logger = require('pino') from "./logger.js"

let headers = { "connection": "keep-alive", "accept": "*/*" }
...
But those aren't our real request headers. So how do we get our request headers from our Next.js app? `getServerSideProps` gets called whenever a request is made to our server. It's called with a `context` which has a bunch of interesting data about the request we can introspect with structured logging!
...
The Logflare Vercel integration guide has an [example Data Studio dashboard](https://logflare.app/guides/vercel-setup#example-dashboard) you can easily copy once you have a data source setup in Data Studio.
...
## Bonus: Log Core Web Vitals to Logflare

Pino is great because we can now setup our app to log from the client in the exact same way we log from the server. To set this up follow our guide on sending Core Web Vitals to Logflare from your Next.js app.
