...
The best thing about Vercel logging is that your log messages are automatically associated with the context of the request because they're embedded in the `lambdaMessage`. This lets you easily correlate the end user experience with our application execution.
...
Let’s walk through the example repository, which is available on GitHub: [pino-logflare-nextjs-vercel-example](https://github.com/Logflare/pino-logflare-nextjs-vercel-example).

You can deploy it on Vercel by clicking the button below:
...
We created a logger ([logger.js](https://github.com/Logflare/pino-logflare-nextjs-vercel-example/blob/main/logger/logger.js)) which sets the fields to be included with every log event in the `base` object when you instantiate Pino.
...
Let's log all our request headers so we can understand which user agents are making requests to our service. `getServerSideProps` gets called whenever a request is made to our server. It's called with a `context` which has a bunch of interesting data about the request we can introspect with structured logging.

```jsx
// pages/index.js

import { prepObjectKeys } from "../logger/utils.js"
...
Notice that we need to modify our request header keys before sending them to Logflare. Logflare uses BigQuery directly as it's storage engine. Logflare turns each field in an event into it's own BigQuery column, and BigQuery has limitation around column names. Specifically, a column name must contain only letters (a-z, A-Z), numbers (0-9), or underscores (_), and it must start with a letter or underscore.

To make our request header object compatible with Logflare and BigQuery, we call `prepObjectKeys` which replaces the dashes with underscores:
...
Now that we're logging some useful data, let’s set up the Logflare Vercel integration to get our newly structured logs going to Logflare.

### Step 1: Install the Integration

Click the button below to install the integration. During the install process, you will sign up for a Logflare account (or sign in if you already have one) and set up a new Logflare source. Logflare `source`s are where your logs will go.
...
Immediately after you set up a Log Drain, you should see logs streaming into your Logflare account.
...
Logflare integrates seamlessly with Google Data Studio. The Logflare Vercel integration guide has an [example Data Studio dashboard](https://logflare.app/guides/vercel-setup#example-dashboard) you can easily copy once you have a data source set up in Data Studio.

### Example Alerts

With Logflare you can use the same Logflare query to define alerts as you use to query. Let’s set up an SMS alert so we get a text message whenever Vercel finishes deploying our changes.
